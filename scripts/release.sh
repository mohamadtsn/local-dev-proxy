...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            exit 0
            ;;
        [0-9]*.*.*) BUMP="$arg" ;;  # exact version like 2.0.0
        *)
...
preflight() {
    ...
    # Remote must be reachable (only when pushing)
...
run_lint() {
    ...
    if shellcheck -x "${BASE_DIR}/bin/devproxy" "${BASE_DIR}/lib/"*.sh "${BASE_DIR}/scripts/"*.sh 2>&1; then
...
main() {
    ...
    echo -e "  ${COLOR_CYAN}│${COLOR_RESET}  ${COLOR_DIM}Push:${COLOR_RESET}     $( $NO_PUSH  && echo "no (--no-push)"  || echo "yes → GitHub Actions will run tests + create release" )"
    ...
    if $DRY_RUN; then
        ...
        if ! $NO_PUSH; then
            echo -e "  ${COLOR_DIM}GitHub Actions is now running tests and creating the release.${COLOR_RESET}"
            echo -e "  ${COLOR_DIM}Watch: https://github.com/mohamadtsn/local-dev-proxy/actions${COLOR_RESET}"
        fi
    fi
    echo ""
}
